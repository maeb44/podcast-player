...
// }
export async function fetchLanding(page = 1) {
    const params = new URLSearchParams({
        page: String(page),
        sort: "recent_published_first",
    });
    const URL = `https://listen-api-test.listennotes.com/api/v2/best_podcasts?${params}`;
    try {
...
export async function fetchInput(searchText) {
    const params = new URLSearchParams({
        q: `${searchText}`,
        type: 'podcast'
    });
    const URL = `https://listen-api-test.listennotes.com/api/v2/search?${params}`;
    try {
        const response = await fetch(URL, {
            method: "GET",
            headers: {
...
//# sourceMappingURL=fetchRecent.js.map